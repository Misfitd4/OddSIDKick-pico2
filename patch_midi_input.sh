...
if '#include "midi_sid.h"' not in s:
    s = s.replace('#include "reSIDWrapper.h"\n', '#include "reSIDWrapper.h"\n#include "midi_sid.h"\n')

if "midi_sid_init();" not in s:
    s = s.replace(
        "uart_set_fifo_enabled(SID_SERIAL_UART, true);\n",
        "uart_set_fifo_enabled(SID_SERIAL_UART, true);\n\n    midi_sid_init();\n"
    )

if "midi_sid_pump_timing();" not in s:
    s = s.replace(
...
if "Source/midi_sid.c" not in s:
    s = s.replace("    Source/SKpico.c\n", "    Source/SKpico.c\n    Source/midi_sid.c\n")

if "tinyusb_device" not in s:
    s = s.replace(
        "    pico_audio_i2s\n)",
        "    pico_audio_i2s\n    tinyusb_device\n    tinyusb_board\n)"
    )

# include dir already has Source, so tusb_config.h is visible
p.write_text(s)
PY

echo "MIDI patch applied."
